...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/yuiiis2022/xxuui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/yuiiis2022/xxuui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_xray_status() {
    count=$(ps -ef | grep "web" | grep -v "grep" | wc -l)
    if [[ count -ne 0 ]]; then
        return 0
    else
        return 1
